...
class ScreenEffect {
	...
	remove(type) {
		...
		if (type in this.effects && !!obj.enabled) {
			...
			if (obj.node) obj.wrapper.removeChild(obj.node);
			else obj.wrapper.classList.remove(type);
		}
	...
	renderTrackingNoise(radius = 2, xmax, ymax) {
		...
		for (let i = 0; i <= num; i++) {
			...
			let y1 = getRandomInt((posy1 += 3), posy2);
			let y2 = getRandomInt(0, (posy3 -= 3));
...
export default function Header() {
	...
	useEffect(() => {
		...
		const config = {
			effects: {
				roll: { enabled: false, options: { speed: 1000 } },
				image: {
					enabled: true,
					options: {
						src: "https://live-production.wcms.abc-cdn.net.au/818ca1b7165f14f54a24aa68e9233a00?impolicy=wcms_crop_resize&cropH=1125&cropW=2000&xPos=0&yPos=0&width=862&height=485",
						blur: 1.2,
					},
				},
		...
		for (const effect in config.effects) {
			...
			f1.add(type, "enabled")
				.name(effect)
				.onChange((bool) => {
					if (bool) screen.add(effect, config.effects[effect].options);
					else screen.remove(effect);
				});

			if (type.options) {
				let folder = effect === "vcr" ? f3 : f2;
	...
	return (
		<div id="screen" ref={screenRef} style={{ width: "100%", height: "400px" }}></div>
	);
}
